...
Este estudo apresentou o desenvolvimento e avaliação de modelos de Redes Bayesianas utilizando os métodos BIC Score e K2 Score para prever a faixa de renda com base em variáveis selecionadas. Os resultados obtidos indicaram que ambos os modelos possuem uma capacidade satisfatória para discriminar entre as classes de renda, alcançando uma AUC próxima de 0.92, o que denota uma forte capacidade de separação. Com acurácias de aproximadamente 89.8\% e valores de Log-Loss em torno de 0.24, ambos os métodos apresentaram previsões precisas e consistentes, demonstrando serem adequados para a tarefa proposta.
A análise comparativa entre os modelos revelou que o BIC tende a favorecer uma estrutura de rede mais simplificada, enquanto o K2 permite maior flexibilidade, gerando uma estrutura ligeiramente mais complexa. Isso sugere que o BIC pode ser mais adequado para cenários onde a simplicidade e a interpretabilidade são fundamentais, enquanto o K2 pode oferecer uma análise mais detalhada e sensível das interações entre variáveis. As curvas ROC ilustraram a eficácia de ambos os modelos em minimizar a taxa de falso positivo, reforçando a confiança nas previsões realizadas.
Apesar dos resultados promissores, o estudo também revelou algumas limitações. Primeiramente, a utilização de amostras reduzidas foi necessária para contornar limitações computacionais, o que pode ter influenciado a robustez dos modelos. Além disso, embora os modelos tenham demonstrado desempenho elevado nas métricas avaliadas, ajustes adicionais nos hiperparâmetros e na seleção de variáveis poderiam potencialmente melhorar ainda mais os resultados.
Para aprimorar o trabalho realizado, a utilização de amostras maiores é uma prioridade para aumentar a robustez dos modelos e assegurar que eles generalizem bem para novos dados. Além disso, experimentos com diferentes métodos de estruturação de rede e ajustes mais detalhados de hiperparâmetros podem fornecer insights valiosos para melhorar o desempenho dos modelos, oferecendo uma base sólida para resultados mais precisos e confiáveis em aplicações futuras.
